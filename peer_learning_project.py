...
def match_students_to_aid(student_id=None, aid_id=None):
    ...
    if not (student_id or aid_id):
        raise ValueError("You must enter either student_id or aid_id.")
...
def allocate_aid(student_id, aid_id, amount):
    ...
    if student["aid_status"] == "Funded":
        conn.close()
        raise ValueError("Student already funded... cannot allocate aid")
    ...
    # Update the student funding status
    c.execute("""
        UPDATE students
